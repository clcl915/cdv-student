...
console.log(
  typeof data !== "undefined"
    ? "seems like it ;-) it comes from the dataManager.js script."
    : "...damnit! let's see what is going wrong in the dataManager.js script."
);
...
function visualizeData(){
  ...
  xAxisGroup.transition().delay(1000).call(xAxis).selectAll("text").attr("font-size", 18); // we adjust this to bring the new axis onto the page
  xAxisGroup.selectAll("line").remove();
...
function resetData() {
  document.getElementById("buttonF").querySelector("p").innerHTML = "*Reset*";
  resetDatapoints();
  visualizeData();
}
document.getElementById("buttonF").addEventListener("click", resetData);